...
import leo.shared.crusades.UnitWarlock;

import java.util.Vector;


public class ScriptWarlock extends ScriptInterceptor {
    private final UnitWarlock warlock;
    ...
    // Constructor
    /////////////////////////////////////////////////////////////////
    public ScriptWarlock(Unit newUnit, Game newGame, Castle newCastle, Castle newEnemyCastle) {
        super(newUnit, newGame, newCastle, newEnemyCastle, true);
        warlock = (UnitWarlock) newUnit;
        unit = newUnit;
        game = newGame;
        castle = newCastle;
        enemyCastle = newEnemyCastle;
        fireball = warlock.getFireball();
    }
    ...
    public void perform() {
        int tries = 0;
        while (!fireball(fireball) && tries < 5) {
            tries++;
        }
    ...
    private void interpretAction(short action, short actor, short target) {
        try {
            ...
            Log.error("ScriptWarlock.interpretAction " + e);
        }
    }
