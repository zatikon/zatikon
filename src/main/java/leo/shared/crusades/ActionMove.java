...
public class ActionMove implements Action {
    ...
    public String perform(short target) {
        ...
        Unit unitAt = owner.getCastle().getBattleField().getUnitAt(target);

        // do it
        short result = owner.getCastle().getBattleField().move(owner, owner.getLocation(), target);

        if (unitAt != null && result != Event.CANCEL) {
            if (unitAt.isPowerUp()) {
                unitAt.die(false, unitAt);
                unitAt.setTeam(owner.getTeam());
                unitAt.setCastle(owner.getCastle());

                // grow if it's targetable by the powerup
                if (unitAt.canTarget(owner)) {
                    owner.grow(unitAt.getPower());
                    // generate powerup image
                    //unitAt.getCastle().getObserver().imageDraw(target, unitAt.getAppearance(),6);
                    owner.getCastle().getObserver().imageDraw(unitAt, unitAt.getLocation(), unitAt.getAppearance(), 6);
                }
                // otherwise banish it
                else {
                    owner.getCastle().add(unitAt);
                    // generate banish effect
                    owner.getCastle().getObserver().abilityUsed(owner.getLocation(), target, Constants.IMG_POOF);
                    owner.getCastle().getObserver().castleAddition(unitAt);
                }
            } else
                unitAt = null;
        }
    ...
    public Vector<Short> getTargets() {
        Vector<Short> targets = owner.getBattleField().getTargets(owner, getTargetType(), getRange(), false, false, false, TargetType.MOVE, owner.getCastle());
