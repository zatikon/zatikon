...
public class ActionDamn implements Action {
  ...
  if (victim == owner) return false;*/
        ...
        for (int i = 0; i < targets.size(); i++) {
            ...
            if ((location.byteValue() == target || (owner.getBattleField().getUnitAt(target).getID() == UnitType.IMP.value() && owner.getBattleField().getUnitAt(target).getOrganic(owner))) && summonOverride(owner.getBattleField().getUnitAt(target)))
                return true;
        }
