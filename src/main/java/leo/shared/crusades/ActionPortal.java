...
public class ActionPortal implements Action {
    ...
    // Constructor
    /////////////////////////////////////////////////////////////////
    public ActionPortal(UnitConjurer newOwner) {
        owner = newOwner;
        targetType = TargetType.LOCATION_AREA;
        range = 3;
        cost = 1;
        hiddenUnit = new UnitPortal(owner.getCastle());
    ...
    public String perform(short target) {
        ...
        UnitPortal portal = new UnitPortal(owner.getCastle());
        portal.setParent(owner);
        owner.getCastle().checkPowerUpPickup(portal, owner.getCastle().getBattleField().getUnitAt(target));
        portal.setLocation(target);
        portal.setBattleField(owner.getCastle().getBattleField());
        owner.getCastle().getBattleField().add(portal);
        owner.getCastle().addOut(owner.getTeam(), portal);

        // Assigns the summon to the owners summoning manager
        owner.getSummonManager().recieveSummon(portal);
        ...
        if (portal.getLocation() == castle.getLocation()) {
            castle.getObserver().endGame(owner.getCastle());
            return portal.getName() + Strings.ACTION_PORTAL_1;
        }
        owner.getBattleField().event(Event.WITNESS_ACTION, owner, portal, getType(), Event.NONE, Event.OK);

        return "" + owner.getName() + Strings.ACTION_PORTAL_2;
    ...
    public String getDescription() {
        return Strings.ACTION_PORTAL_3;
    }
    ...
    public String getCostDescription() {
        return Strings.ACTION_PORTAL_4;
    }
    ...
    public String getName() {
        return Strings.ACTION_PORTAL_5;
    }
