...
public class ActionMark implements Action {
    ...
    public ActionMark(Unit newOwner) {
        ...
        if (owner.getID() == UnitType.CONSPIRATOR.value()) {
            range = 4;
        } else {
            range = 4;
        }
    ...
    public boolean validate(short target) {
        ...
        if (owner.getID() == UnitType.CONSPIRATOR.value()) {
            ...
            if (owner.getBattleField().getUnitAt(target) == null || owner.getBattleField().getUnitAt(target).getID() == UnitType.NONE.value())
                return false;
        }
    ...
    public Vector<Short> getTargets() {
        ...
        if (owner.getID() == UnitType.CONSPIRATOR.value()) {
            ...
            while (it.hasNext()) {
                Unit victim = it.next();
                if (victim.getID() == UnitType.NONE.value()) // Conspirator won't target summons or doppelganger twin since wouldn't have effect anyways
