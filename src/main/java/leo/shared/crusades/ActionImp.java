///////////////////////////////////////////////////////////////////////
// Name: ActionImp
// Desc: Summon a nastly little imp
...
public class ActionImp implements Action {
    ...
    // Constructor
    /////////////////////////////////////////////////////////////////
    public ActionImp(UnitSummoner newOwner) {
        owner = newOwner;
        targetType = TargetType.LOCATION_AREA;
        range = 1;
        cost = 1;
        hiddenUnit = new UnitImp(owner.getCastle());
    }
    ...
    public String perform(short target) {
        ...
        UnitImp imp = new UnitImp(owner.getCastle());
        imp.setParent(owner);
        owner.getCastle().checkPowerUpPickup(imp, owner.getCastle().getBattleField().getUnitAt(target));
        imp.setLocation(target);
        imp.setBattleField(owner.getCastle().getBattleField());
        owner.getCastle().getBattleField().add(imp);
        owner.getCastle().addOut(owner.getTeam(), imp);

        // Assigns the summon to the owners summoning manager
        owner.getSummonManager().recieveSummon(imp);
        ...
        if (imp.getLocation() == castle.getLocation()) {
            castle.getObserver().endGame(owner.getCastle());
            return imp.getName() + Strings.ACTION_IMP_1;
        }

        owner.getBattleField().event(Event.WITNESS_ACTION, owner, imp, getType(), Event.NONE, Event.OK);

        return "" + owner.getName() + Strings.ACTION_IMP_2 + (BattleField.getX(target) + 1) + ", " + (BattleField.getY(target) + 1);
